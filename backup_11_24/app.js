// app.js — unified dataflow for physical + virtual keyboard.
...
// ============================================================================
// Editor helpers (ONE dataflow: both physical & virtual go through same logic)
// ============================================================================
...
// Global sticky modifier state (applies to both physical and virtual keyboard)
let modShift = false;
let modCtrl  = false;
let modAlt   = false;

// Optional: show modifier state in status (for debugging/feedback)
function updateModifierStatus() {
  ...
  // Don’t overwrite other important messages if you don’t want to:
  // we’ll just append this when we set status text in doTyping, etc.
  return modStr;
}

// Shift mapping similar to US QWERTY
function applyShift(label) {
...
// Normalize various labels / key values into canonical "key" names.
// Used for BOTH physical and virtual keyboards.
function normalizeKeyName(key) {
  // Some browsers use "Spacebar" or " " etc.
  if (key === ' ' || key === 'Spacebar' || key === 'Space') return ' ';
  if (key === 'Enter' || key === 'Return') return 'Enter';
  if (key === 'Backspace' || key === 'Delete') return 'Backspace';

  // Modifier variants
  if (key === 'ShiftLeft' || key === 'ShiftRight' || key === '⇧' || key === 'SHIFT') return 'Shift';
  if (key === 'ControlLeft' || key === 'ControlRight' || key === 'Ctrl' || key === 'CONTROL' || key === 'Control') return 'Control';
  if (key === 'AltLeft' || key === 'AltRight' || key === 'Alt' || key === 'Option' || key === '⌥') return 'Alt';

  return key;
}

// Unified handler: ANY key (physical or virtual) passes through here.
function handleKeyFromAnySource(rawKey) {
  const key = normalizeKeyName(rawKey);

  // Sticky modifier toggles
  if (key === 'Shift') {
    modShift = !modShift;
    statusEl.textContent = `Shift ${modShift ? 'ON' : 'OFF'}${updateModifierStatus()}`;
    return;
  }
  if (key === 'Control') {
    modCtrl = !modCtrl;
    statusEl.textContent = `Ctrl ${modCtrl ? 'ON' : 'OFF'}${updateModifierStatus()}`;
    return;
  }
  if (key === 'Alt') {
    modAlt = !modAlt;
    statusEl.textContent = `Alt ${modAlt ? 'ON' : 'OFF'}${updateModifierStatus()}`;
    return;
  }
  ...
  // You could add Ctrl-based shortcuts here using modCtrl (e.g. Ctrl+Z, Ctrl+C...)
  ...
  // For now: ignore other keys (Tab, arrows, etc.) or add behavior if needed.
}
...
// Map a virtual key label (from overlay) to a "key" string to feed into handleKeyFromAnySource.
function mapLabelToKey(label) {
  // Common special keys in your layout
  if (label === 'Space') return ' ';
  if (label === '↩' || label === 'Enter') return 'Enter';
  if (label === '⌫') return 'Backspace';

  // Modifiers (many possible label variants)
  if (label === 'Shift' || label === '⇧' || label === 'SHIFT') return 'Shift';
  if (label === 'Ctrl' || label === 'Control' || label === 'CTRL') return 'Control';
  if (label === 'Alt' || label === 'Option' || label === '⌥') return 'Alt';

  // Default: treat label itself as the key (e.g., 'A', 'b', '1', etc.)
  return label;
}

// ============================================================================
// Typing (only after calibration freeze) — unified path
// ============================================================================
...
function doTyping(tips, quad) {
  ...
  // Debounce: avoid flooding if finger hovers on same key
  if (label === lastKey && (now - lastKeyTime) < 120) return;
  lastKey = label;
  lastKeyTime = now;

  const key = mapLabelToKey(label);
  handleKeyFromAnySource(key);
}
...
(async function main() {
  ...
  try {
    ...
    // Editor helpers buttons
    focusEditorBtn.onclick = () => editor.focus();
    clearEditorBtn.onclick = () => setValueAndCaret('', 0);

    // PHYSICAL keyboard → same handler as virtual keyboard.
    editor.addEventListener('keydown', (e) => {
      // We take full control of editing behavior so that
      // physical and virtual keyboard behave identically.
      e.preventDefault();
      handleKeyFromAnySource(e.key);
    });
...
// ============================================================================
// Calibration helpers + camera start/stop
// ============================================================================
...
async function start(startFn, ...args) {
  btnStartRear.disabled = true;
  btnStartFront.disabled = true;
  ...
  try {
    ...
    btnStartRear.disabled = false;
    btnStartFront.disabled = false;
  }
}
...
function mainLoop() {
  ...
  if (!frozen && tips && tips.length) {
    ...
    if (idx.length >= 2) {
      ...
      if (elapsed >= 10) {
        ...
        statusEl.textContent = `Frozen${updateModifierStatus()}`;
      } else {
  ...
  // Typing only after freeze (match A) — unified dataflow
  if (frozen && tips && tips.length) {
    doTyping(tips, quad);
  }
}
